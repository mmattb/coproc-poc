...
TODO: describe stopping conditions
...
M1 No-coadapt  & 0.021136                                                          &                                                                  &                                                                      & 102.75\%           & 102.75\%                                                         \\ \hline
...
patient has lost some of the neural machinery responsible for representing the task.
...
In this case, the co-processor is effectively unable to improve on existing performance,
...
recovery, or results in greater results than natural recovery, since the simulation
is not designed to answer that question.
...
* Initially, performance improves orders of magnitude more quickly than later. 50-75\% recovery during this phase. This appears to be the co-processor learning the task structure (e.g. not to move the hand until after the hold signal is lifted).
* Later: fine tuning and object class separation. Learning task structure.

\subsection{Experiment 3: F5 and M1 connectivity lesion}
As in the other cases, the co-processor's loss in this experiment improved
quickly, initially, but takes a long time to refine. During that refinement,
object differentiation occurs. Note that in this experiment, no information
can propagate forward to the M1 module, since we have completely severed the
input connections to that module. It is the sole responsibility of the
co-processor to perform that forward-propagation. As a result, co-adaptation
only affects the M1 module. The co-processor's learning algorithm successfully
adapted to this case, where the observed brain activity was stationary, but
the mapping between stimulation and task performance was not.
...
Note that the object separation class separation metric $S$ returned to
a healthy value towards the end of the training, indicating that the
learned solution did not simply refine the stereotyped grasp. That in
turn demonstrates the co-processor's ability to propagate information
``around'' the simulation lesion.
