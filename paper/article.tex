...
lesioning the outputs of neurons leaves enough remaining degrees of freedom
that a nearly full recovery can occur, unless so many neurons are lesioned
that no stimulation could be effective. We go into this further in the
...
the MSE loss between the EN's predicted output, and target output, to the CPN.
...
gradients which train the CPN to output more useful stimulus. We discovered that
...
on white noise stimulation - to orders of magnitude lower loss than the task loss -
while at the same time backpropagation through it yields gradients which are not
useful for training the CPN. As a result, the CPN exhibits unstable training.
We have thus far been unable to discover a way to measure and optimize for this second
property, though pursuing it may be a fruitful subject of future ML research. Instead
of attempting to train an EN directly to exhibit this property, we discovered a
training regime that reliably makes it appear.
...
to great predictive power on a set of stimulation examples, but that the effective
...
network being stimulated. As a result, the hypersurface which it has fit to the training
examples may exhibit gradients not reflective of reality.

First, and most significantly, we structure the training data set for each EN training epoch
in a very specific way. Each epoch, we include examples from the current CPN, the current CPN
with Gaussian-distributed mean-0 noise added to its parameters, and examples of white noise
stimulation. We developed this structure under the believe that we needed to cover a sufficient
...
that the EN is to specialized to the local area of CPN parameter space, but we then solve
that problem using EN retraining, interleaved with the CPN, as we will explain further below.
...
Examples produced by the current CPN                                                                                                       & 10\%                           \\ \hline
\begin{tabular}[c]{@{}l@{}}Examples produced by a copy of the current CPN,\\ with mean-0 random noise added to its parameters\end{tabular} & 60\%                           \\ \hline
Examples of white noise stimulation                                                                                                        & 30\%                           \\ \hline
\end{tabular}
\end{table}

Second, we use of PyTorch's $AdamW$ optimizer, which includes an option for weight regularization,
and a carefully chosen learning rate schedule. Our intuition is to leverage regularization as
one of the standard mitigations for overfit. To converge, the learning rate schedule also needs
...
Once we discovered an effective way to train an EN, CPN training was straightforward. For this phase,
...
algorithm on the basis of those two. We train the two in alteration, creating
...
The key challenge we face is determining when an EN is no longer suitable for training the
...
Unfortunately, because we expire the EN rather than retraining it, and because we need
to expire it so often, training is less efficient than it otherwise could be. We explore
this some in Section \ref{sec:results}. In the next subsection, we illustrate one way we
mitigate training inefficiency.

TODO: pseudocode

\subsubsection{Optimization: data reuse}
One obvious
...
\begin{itemize}
	...
	\item 100\% connectivity lesion between the F5 and M1 modules without brain co-adpatation
	\item 50\% M1 loss
\end{itemize}
